...
async function runFullTestSuite() {
  ...
  const tools = [
    ...
    { name: 'Password Generator', path: 'password-generator.html', test: async (p) => {
      const hasInput = await p.$('#passwordOutput') !== null;
      const hasBtn = await p.$('#generateBtn') !== null;
      return { overallPass: hasInput && hasBtn, errors: [], message: '✅ Password Generator ready' };
    }}
  ];
...
async function testQRGenerator(page) {
  ...
  // Fill input
  await page.waitForSelector('#qrText', { timeout: 10000 });
  await page.type('#qrText', 'Test QR Code');
...
async function testTextToSpeech(page) {
  const errors = [];
  const hasInput = await page.$('#textInput') !== null;
  const hasSpeakBtn = await page.$('button[onclick="speakText()"]') !== null;
  const hasListenBtn = await page.$('#micBtn') !== null;

  return {
    overallPass: hasInput && hasSpeakBtn && hasListenBtn,
    errors: (hasInput && hasSpeakBtn && hasListenBtn) ? [] : ['Missing TTS/STT UI elements'],
    message: (hasInput && hasSpeakBtn && hasListenBtn) ? '✅ Text to Speech UI ready' : '❌ UI incomplete'
  };
}
...
async function testCalculator(page) {
  ...
  // Test basic addition
  await page.waitForSelector('#basicNum1', { timeout: 5000 });
  await page.type('#basicNum1', '10');
  await page.type('#basicNum2', '5');
  await page.click('#addBtn');
  const result = await page.$eval('#result', el => el.innerText);
  if (!result.includes('15')) errors.push(`Basic math: expected 15, got ${result}`);

  // Test currency (use static rate)
  await page.select('#currencyFrom', 'USD');
  await page.select('#currencyTo', 'INR');
  await page.type('#currencyAmount', '100');
  await page.click('button[onclick="convertCurrency()"]');
  await new Promise(resolve => setTimeout(resolve, 500));
  const resultText = await page.$eval('#result', el => el.innerText);
  if (!resultText.includes('8300')) errors.push('Currency conversion incorrect');
...
async function testPDFToWord(page) {
  ...
  // Check docx library (JSZip used for docx)
  const docxLoaded = await page.evaluate(() => typeof JSZip !== 'undefined');

  if (!pdfjsLoaded) errors.push('pdf.js not loaded');
  if (!docxLoaded) errors.push('JSZip not loaded');
...
async function testPassportPhotoMaker(page) {
  ...
  const hasImgInput = await page.$('#fileIn') !== null;
  ...
  return {
    overallPass: hasImgInput && hasGoBtn && hasColorSelect && mediaPipeLoaded,
    errors,
...
async function testImageFormatConverter(page) {
  ...
  const hasInput = await page.$('#imgInput') !== null;
